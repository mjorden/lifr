...
`tinytex::install_tinytex()`) for the PDF report.
...
<img src="man/figures/README-plot-all-1.png" width="100%" />
...
`qc_compare()` shows exactly what changed, with the edited depths in
red:
...
<img src="man/figures/README-qc-1.png" width="100%" />
...
- `vignette("data-format")`: the raw file formats and what import does
  with them.
- `vignette("editing")`: masking artifacts, edit plans, and the audit
  trail.
- `vignette("plotting")`: a gallery of every plot with its options.
- `vignette("reporting")`: `process_site()`, the report tabs, and output
  files.
